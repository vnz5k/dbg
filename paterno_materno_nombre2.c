...
// Función que crea un nuevo cliente con los datos proporcionados
...
// Función que inserta un nuevo cliente en la lista enlazada
Cliente *insertar_cliente(Cliente *lista, Cliente *nuevo) {
    nuevo->siguiente = lista;
    return nuevo;
}

// Función que busca un cliente en la lista enlazada por su RUT
...
// Función que elimina todos los elementos de la lista enlazada
...
// Función que procesa las entradas de los clientes desde un archivo de entrada y
// crea archivos de salida con entradas asignadas y en espera
void procesar_entrada(char *archivo_entrada, char *archivo_asignadas, char *archivo_espera) {
    ...
    while (fscanf(entrada, "%[^,],%[^,],%d%*c", rut, nombre, &entradas_solicitadas) == 3) {
        ...
        if (cliente == NULL) {
            ...
            // Si el cliente solicitó hasta 2 entradas
            if (cliente->entradas_solicitadas <= 2) {
                int entradas_a_asignar = cliente->entradas_solicitadas - cliente->entradas_asignadas;
                // Si hay suficientes entradas pendientes para asignar todas las solicitadas
            ...
            // Si el cliente solicitó más de 5 entradas
            } else {
        ...
        // Si al cliente no se le asignaron todas las entradas solicitadas, añadirlo a la lista de espera
    ...
    // Escribir los clientes y sus entradas asignadas en el archivo de asignadas
    ...
    // Escribir los clientes en espera y sus entradas solicitadas en el archivo de espera
    actual = espera;
    while (actual != NULL) {
...
int main(int argc, char *argv[]) {
    ...
    // Procesar el archivo de entrada y generar los archivos de salida
